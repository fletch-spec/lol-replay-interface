...
Shipped as specified. Every Can't Skip item is in and was verified against a
live replay (`CLASSIC:1516`), not just read back from code.
...
either. Fell through to the brief's own documented fallback, `gameMode` +
`length`, and pulled it out into a shared `replayIdentity()` so brief 004's
event cache and 005's cues can't drift apart. This turned out to be exercised
for real: a different replay was loaded partway through testing and identity
moved from `CLASSIC:2196` to `CLASSIC:1516` on its own, with the right cue set
following it.
...
clears so it re-derives from wherever you scrubbed to.
...
observe, so a copy-only export can quietly do nothing. The button reports which
path it took.

**Verified live:** M places (double-tap within 0.5s swallowed); N/B step
forward and back landing paused at `t - 2s` each time; `[`/`]`/`L` arm a loop
that ran 206→212→wrap→200 without ever passing B; Esc clears; notes edit in
